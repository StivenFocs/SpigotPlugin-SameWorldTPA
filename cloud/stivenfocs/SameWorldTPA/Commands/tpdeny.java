...
public class tpdeny implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String s, String[] args) {
        if (Vars.hasPermission("tpdeny", sender)) {
            if (sender instanceof Player) {
                ...
                if (args.length > 0) {
                    ...
                    if (request_sender != null) {
                        ...
                        if (request != null) {
                            request.deny();
                            Vars.sendString(Vars.setPlaceholders(Vars.request_denied, request_sender, p), sender);
                            Vars.sendString(Vars.setPlaceholders(Vars.your_request_denied, request_sender, p), request_sender);
                        } else {
                            Vars.sendString(Vars.setPlaceholders(Vars.no_requests_from_this_player, request_sender, p), sender);
                        }
                    } else {
                    ...
                    if (received_requests.size() == 1) {
                        ...
                        request.deny();
                        Vars.sendString(Vars.setPlaceholders(Vars.request_denied, request.sender, request.recipient), sender);
                        Vars.sendString(Vars.setPlaceholders(Vars.your_request_denied, request.sender, request.recipient), Bukkit.getPlayer(request.sender));
                    } else {
                        Vars.sendString(Vars.tpdeny_usage, sender);
                    }
                }
            } else {
