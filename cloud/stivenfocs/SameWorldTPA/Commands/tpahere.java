            ...
            if (sender instanceof Player) {
                ...
                if (args.length > 0) {
                    ...
                    if (recipient != null) {
                            ...
                            if (RequestsHandler.doesPlayerAllowRequests(recipient.getUniqueId())) {
                                if (p.getWorld().getName().equals(recipient.getWorld().getName())) {
                                    ...
                                    if (existing_request != null) {
                                        if (existing_request.request_type.equals(RequestType.TPAHERE)) {
                                    ...
                                    RequestsHandler.createRequest(p.getUniqueId(), recipient.getUniqueId(), RequestType.TPAHERE, Vars.request_duration, Vars.teleport_cooldown);
                                    Vars.sendString(Vars.setPlaceholders(Vars.tpahere_request_sent, p, recipient), sender);
                                    Vars.sendString(Vars.setPlaceholders(Vars.tpahere_request_received, p, recipient), recipient);
                                    Vars.playSound("tpahere_received", recipient);
                                } else {
                        ...
                        Vars.sendString(Vars.unknown_player.replaceAll("%recipient_name%", args[1]).replaceAll("%recipient_displayname%", args[1]), sender);
                    }
                } else {
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String s, String[] args) {
        ...
        if (args.length == 1) {
            if (args[0].equalsIgnoreCase("")) {
                if (Vars.hasPermission("tpahere", sender)) {
